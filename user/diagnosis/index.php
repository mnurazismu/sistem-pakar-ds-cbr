...
$query_pertanyaan = "SELECT * FROM pertanyaan ORDER BY urutan ASC";
        ...
        <div class="p-4 border-2 border-gray-200 rounded-lg">
            ...
            <div class="mb-8">
                ...
                <div class="w-full bg-gray-200 rounded-full h-2.5">
                    <div class="bg-blue-600 h-2.5 rounded-full step-animation" 
                         id="progressBar" 
            ...
            <?php if (isset($_SESSION['flash_message'])): ?>
                <div class="mb-4 p-4 rounded-lg <?= $_SESSION['flash_message']['type'] === 'error' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' ?>">
            ...
            <form id="diagnosisForm" action="process.php" method="POST" class="space-y-6">
                <?php foreach ($pertanyaan as $index => $p): ?>
                    <div class="question-card <?= $index === 0 ? 'active' : 'inactive' ?>" 
                         data-step="<?= $index + 1 ?>">
                        <div class="bg-white p-6 rounded-lg shadow-lg border border-gray-100">
                            <div class="flex items-center mb-4">
                                <span class="flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 text-blue-800 font-semibold mr-3">
                                    <?= $index + 1 ?>
                                </span>
                                    ...
                                    <?php foreach ($pilihan as $pil): ?>
                                        <div class="flex items-center p-3 rounded-lg hover:bg-gray-50 transition-colors">
                                            <input type="radio" 
                                                   ...
                                                   value="<?= $pil['id_pilihan'] ?>"
                                                   id="pilihan_<?= $pil['id_pilihan'] ?>"
